...
contract FruitfulPlayer is GetERC20 {
  ...
  function mintGenesis(uint256 tokenId) public {
...
contract VRFFacetTest is GetERC20, DSTest {
  ...
  function setUp() public virtual {
    ...
    // weth = WETH(0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619);
    // deploy contracts
    vrfFacet = new VRFFacet("Fruitful Metaverse Base Character", "FID", "TESTURI");
    // create haibara's account
    haibara = new FruitfulPlayer(vrfFacet, dlc, link);
    // payable(address(this)).transfer(1);
    emit log_named_uint("Player's balance", address(haibara).balance);
    ...
    getERC20Quick(0x53E0bca35eC356BD5ddDFebbD1Fc0fD03FaBad39, 10e18);

    /// transferring the LINK tokens to the VRFFacet contract
    link.transfer(address(vrfFacet), 10e18);
